...
clip_model.eval()
clip_model_path = base_path / f"/home/banik8/composed image retrieval/Fashion_recommender/clip_finetuned_on_fiq_RN50x4/saved_models/tuned_clip_best.pt"        
...
# --- Multi-GPU support for combiner ---
if torch.cuda.device_count() > 1:
    combiner = torch.nn.DataParallel(combiner)
...
resume_from_checkpoint = False  # Always start from scratch for a fresh, compatible combiner
...
def sample_hard_negative(dataset, anchor_idx, attribute):
    """
    Given an anchor index and attribute, sample a hard negative index from the dataset that does NOT match the attribute.
    """
    # Get indices that do NOT mention the attribute
    neg_indices = dataset.filter_by_attribute(attribute, include=False)
    # Exclude the anchor itself if present
    neg_indices = [idx for idx in neg_indices if idx != anchor_idx]
    if not neg_indices:
        # Fallback: random negative
        neg_indices = list(range(len(dataset)))
        neg_indices.remove(anchor_idx)
    return np.random.choice(neg_indices)
...
for epoch in range(start_epoch, num_epochs):
    ...
    for idx, (reference_images, target_images, captions) in enumerate(train_bar):
        ...
        with torch.no_grad():
            reference_image_features = clip_model.module.encode_image(reference_images).float() if hasattr(clip_model, 'module') else clip_model.encode_image(reference_images).float()
            target_image_features = clip_model.module.encode_image(target_images).float() if hasattr(clip_model, 'module') else clip_model.encode_image(target_images).float()
            text_features = clip_model.module.encode_text(text_inputs).float() if hasattr(clip_model, 'module') else clip_model.encode_text(text_inputs).float()
        ...
        # --- Hard Negative Sampling Example ---
        # For demonstration, use the first attribute word in the caption as the attribute (customize as needed)
        batch_hard_negatives = []
        for b in range(images_in_batch):
            # Fix: Add proper bounds checking for captions
            if b < len(captions) and captions[b] and len(captions[b]) > 0:
                attribute = captions[b][0].split()[0] if captions[b][0] else ''
            else:
                attribute = ''  # Default attribute if no caption available
            
            # Use a simple random negative sampling instead of hard negative for now
        ...
        batch_hard_negatives = torch.cat(batch_hard_negatives, dim=0).to(device)
        with torch.no_grad():
            negative_features = (
                clip_model.module.encode_image(batch_hard_negatives).float()
                if hasattr(clip_model, 'module')
                else clip_model.encode_image(batch_hard_negatives).float()
            )

        with torch.cuda.amp.autocast():
            predicted_features = combiner.module.combine_features(reference_image_features, text_features) if hasattr(combiner, 'module') else combiner.combine_features(reference_image_features, text_features)
            target_features = F.normalize(target_image_features, dim=-1)
            negative_features = F.normalize(negative_features, dim=-1)
            # Example: Triplet loss with hard negatives
            triplet_loss = nn.TripletMarginLoss(margin=0.2)
            loss = triplet_loss(predicted_features, target_features, negative_features)
    ...
    if epoch % validation_frequency == 0:
        ...
        for dress_type in idx_to_dress_mapping:
            recall_at10, recall_at50 = fashioniq_val_retrieval(
                ...
                combining_function=combiner.module.combine_features if hasattr(combiner, 'module') else combiner.combine_features,
