# -*- coding: utf-8 -*-
"""
Utility functions and dataset class for FashionIQ.
"""
...
base_path = Path("/home/banik8/composed image retrieval/Fashion_recommender/fashion-iq-cleaned")
...
class FashionIQDataset(Dataset):
    ...
    The dataset can be used in 'relative'  mode:
    ...
    def __init__(self, split: str, dress_types: List[str], mode: str, preprocess: callable):
        ...
        if mode not in ['relative','classic']:
            raise ValueError("mode should be in ['relative']")
    ...
    def filter_by_attribute(self, attribute: str, include: bool = True):
        """
        Returns indices of triplets whose captions (modification text) mention (or do not mention) the given attribute.
        :param attribute: Attribute/category to filter by (e.g., 'long sleeve')
        :param include: If True, include samples with the attribute; if False, exclude them.
        :return: List of indices
        """
        filtered_indices = []
        for idx, triplet in enumerate(self.triplets):
            captions = triplet.get('captions', [])
            # Check if any caption mentions the attribute
            found = any(attribute.lower() in cap.lower() for cap in captions)
    ...
    def __getitem__(self, index):
        try:
            if self.mode == 'relative':
                ...
                if self.split == 'train':
                    reference_image_path = base_path / f"{reference_name}.png"
                    target_name = self.triplets[index]['target']
                    target_image_path = base_path / f"{target_name}.png"
                    
                    # Check if both images exist
                    if not reference_image_path.exists() or not target_image_path.exists():
                        print(f"Warning: Missing image files for triplet {index}. Reference: {reference_name}, Target: {target_name}")
                        return None
                    ...
                    if not reference_image_path.exists():
                        print(f"Warning: Missing image file for test sample {index}. Reference: {reference_name}")
                        return None
                    reference_image = self.preprocess(PIL.Image.open(reference_image_path))
                ...
                image_path = base_path / f"{image_name}.png"
                if not image_path.exists():
                    print(f"Warning: Missing image file for classic sample {index}. Image: {image_name}")
                    return None
