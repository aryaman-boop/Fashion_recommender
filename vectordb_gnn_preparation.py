...
base_path = Path("/home/banik8/composed image retrieval/Fashion_recommender")
output_dir = Path("vector_db_gnn_outputs")
output_dir.mkdir(exist_ok=True)

# Load the fine-tuned weights
checkpoint = torch.load("/home/banik8/composed image retrieval/Fashion_recommender/clip_finetuned_on_fiq_RN50x4/saved_models/tuned_clip_best.pt", map_location=device)
...
categories = ['dress', 'shirt', 'toptee']  # Process all categories
...
for category in categories:
    ...
    triplet_list = []
    for split in splits:
        enriched_path = base_path / "data/json_files" / f"cap.{category}.{split}.json"
    ...
    for image_id in all_image_ids:
        for ext in [".jpg", ".png", ".jpeg"]:
            img_path = base_path / "fashion-iq-cleaned" / f"{image_id}{ext}"
            if img_path.exists():
    ...
    for i in tqdm(range(0, len(image_ids), batch_size), desc="Image Embeddings"):
        ...
        with torch.no_grad():
            if isinstance(clip_model, torch.nn.DataParallel):
                batch_embs = clip_model.module.encode_image(batch_imgs).cpu().numpy()
            else:
    ...
    for triplet in tqdm(triplet_list, desc="Text Embeddings"):
        ...
        with torch.no_grad():
            if isinstance(clip_model, torch.nn.DataParallel):
                text_emb = clip_model.module.encode_text(text).cpu().numpy()[0]
            else:
